    ...
    function(_result)
      vim.schedule(function()
        ...
        if self._waiting_for_response then
          vim.notify("Lobs: Disconnected — response paused on server. Reconnect, then use :LobsResume.", vim.log.levels.WARN)
...
function M:_on_message(raw)
  ...
  if t == "session.opened" then
    ...
    self._resume_required = msg.resumeRequired == true
    ...
    if self._resumed_session and old_key == msg.sessionKey then
      if self._resume_required then
        vim.notify("Lobs: reconnected — response paused. Use :LobsResume to continue.", vim.log.levels.WARN)
      else
  ...
  elseif t == "chat.status" then
    ...
    elseif (status == "queued" or status == "resuming") and self._handlers.on_thinking then
      self._handlers.on_thinking()
    end
...
function M:resume_pending()
  if not self._connected or not self.session_key then
    vim.notify("Lobs: not connected", vim.log.levels.WARN)
    return false
  end

  if not self._resume_required then
    vim.notify("Lobs: no paused response to resume", vim.log.levels.INFO)
    return false
  end
  ...
  self._resume_required = false
  self:_start_stall_timer()
  if self._handlers.on_resume then
    self._handlers.on_resume()
  end
  self:_send(protocol.session_resume({
    sessionKey = self.session_key,
  }))
  vim.notify("Lobs: resuming paused response...", vim.log.levels.INFO)
  return true
end
